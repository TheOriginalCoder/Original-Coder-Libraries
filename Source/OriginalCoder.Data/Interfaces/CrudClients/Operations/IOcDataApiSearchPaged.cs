...
namespace OriginalCoder.Data.Interfaces.CrudClients.Operations
{
    ...
    [PublicAPI]
    public interface IOcDataApiSearchPaged<out TEntity, in TSearchParams>
        where TEntity : class
        where TSearchParams : class
    {
        IOcApiResult<IReadOnlyList<TEntity>> Get(TSearchParams searchParams, int qtySkip, int qtyReturn, IReadOnlyList<(string columnName, bool ascending)> sortOrder = null);
    }
}